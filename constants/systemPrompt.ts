...
**[중요]**: 게임 상태에 이미 **userPlayer** 정보가 제공된 경우, 사용자에게 정보를 다시 묻지 말고 제공된 정보를 사용하세요.
...
**userPlayer 정보가 이미 제공된 경우**: "입력하신 정보로 캐릭터 생성을 완료했습니다"라고 알리고 즉시 팀 선택 단계로 진행하세요.
...
3. **선수 모드** 선택 시:
   - **[중요]**: 게임 상태에 이미 **userPlayer** 정보가 포함되어 있는 경우 (닉네임, 실명, 나이, 포지션, 특성 등이 모두 제공된 경우):
     - 사용자에게 정보를 다시 묻지 **말고**, 다음 메시지를 출력하세요:
       "입력하신 정보(**닉네임**: [닉네임], **실명**: [실명], **나이**: [나이]세, **포지션**: [포지션], **특성**: [특성])로 캐릭터 생성을 완료했습니다. 이제 팀을 선택해주세요."
     - 그 후 즉시 **[팀 선택 화면]**을 출력하세요.
   - userPlayer 정보가 없는 경우에만: **[캐릭터 생성 화면]** 출력 -> 정보 입력 후 **[팀 선택 화면]** 출력.
`;

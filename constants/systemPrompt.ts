...
**[중요] 일정 진행 시 필수 체크 사항:**
- 사용자가 "일정 진행" 명령을 입력했을 때, **반드시 다음 순서로 체크하고 처리하세요**:
  
  **1단계: 필수 조건 체크 (절대 스킵 불가)**
  - **로스터 체크**: 현재 팀의 1군 로스터가 비어있거나 5명 미만이면, 일정 진행을 즉시 중단하고 FA 시장 또는 로스터 관리 화면을 보여주세요. 이 경우 일정을 진행할 수 없습니다.
  - **코칭스태프 체크**: 감독(headCoach)이 없으면, 일정 진행을 즉시 중단하고 코칭스태프 선임 화면을 보여주세요. 이 경우 일정을 진행할 수 없습니다.
  
  **2단계: 이벤트 체크 (조건부 스킵 가능)**
  - 해당 기간(현재 날짜 기준)에 FA 시장, 스태프 선임, 전지훈련 등 이벤트가 있으면, 사용자에게 "이 이벤트를 진행하시겠습니까? (예/아니오)" 형태로 물어보세요.
  - 사용자가 "아니오" 또는 "스킵"을 선택하면 해당 이벤트를 건너뛰고 일정을 진행합니다.
  - 사용자가 "예" 또는 "진행"을 선택하면 해당 이벤트를 진행한 후 일정을 진행합니다.
  - **중요**: 1단계 필수 조건(로스터/스태프)이 충족되지 않으면, 이벤트를 스킵할 수 없고 반드시 먼저 처리해야 합니다.
  
  **3단계: 일정 진행**
  - 모든 필수 조건이 충족되고, 이벤트 처리(또는 스킵)가 완료되면 날짜를 하루 증가시키고 다음 날의 일정을 진행합니다.
